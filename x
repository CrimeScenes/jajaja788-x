...
local function UpdateFOV()
    ...
    local success, errorMsg = pcall(function()
        if Circle then
            Circle.Visible = shared.Forbidden.PointAssist.Enabled
...
RunService.RenderStepped:Connect(function()
    if IsTargeting and TargetPlayer and TargetPlayer.Character then
        ...
        if BodyPart then
            ...
            if shared.Forbidden.PointAssist.Align then
...
local function getDynamicFov(targetPosition)
    ...
    -- Updated to match the new shared.Forbidden table structure
    if distance <= shared.Forbidden.Ranges.CloseDistance then
        return shared.Forbidden.Silent.BoundingSizes.CloseSize
    elseif distance <= shared.Forbidden.Ranges.MidDistance then
        return shared.Forbidden.Silent.BoundingSizes.MidSize
    elseif distance <= shared.Forbidden.Ranges.FarDistance then
        return shared.Forbidden.Silent.BoundingSizes.FarSize
    else
        return shared.Forbidden.Silent.BoundingSizes.FarSize
    end
end
...
local function updateFov(TargetPlayer)
    ...
    if IsTargeting and TargetPlayer then  
        ...
        elseif settings.FovShape == "Triangle" then
            ...
            Fov.Position = V2(Mouse.X, Mouse.Y + (game:GetService("GuiService"):GetGuiInset().Y))
        ...
        if closestPlayer then
            ...
            if remoteEvent then
                if Argument then
                    ...
                    -- Handle case when Argument is nil
                end
            else
                -- Handle case when remoteEvent is nil
            end
        end
    end
end)
...
UserInputService.InputEnded:Connect(function(input, isProcessed)
    if input.KeyCode == Enum.KeyCode[shared.Forbidden.Target.Keybind:upper()] and CamLock.Normal.mode == "hold" then
